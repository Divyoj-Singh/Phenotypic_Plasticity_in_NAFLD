...
%%
ap = 6; %describes the index of parameter for which the bifurcation is drawn using the init_EP_EP function. Currently, ap=1, thus bifurcation parameter is s (SNAIL levels)
handles = feval(@nafld_wild_type);
tspan = 0:1:5000;

% initial condition
x_start = [1000,1000,1,1]%(rand(1,4))*1000
I = 15000;

%%% production rate
g_HNF4A = 0.4540700422860051; %g/k = 8
g_HNF1A = 0.05705569053323611; %g/k = 1
g_PPARG = 0.06283939658384676; %g/k = 
g_SREBF1= 1.1058054056141339; %g/k = 7.3

% Degradation Rate
k_HNF4A = 0.049%0.049119799999999997;   %% #% 0.0479
k_HNF1A = 0.049445600000000006; %% #% 0.05
k_PPARG = 0.2;%0.07670399999999999;  %% #% 0.06
